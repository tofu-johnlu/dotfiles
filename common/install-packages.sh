...
install_factory_ai() {
    ...
    if command -v factory >/dev/null 2>&1; then
        echo " [✓]  Factory AI CLI is already installed"
    else
        printf " [ ]  Installing Factory AI CLI..."
        if curl -fsSL https://app.factory.ai/cli | sh > /tmp/factory_install_$$.log 2>&1; then
            printf "\r [✓]  Installing Factory AI CLI\n"
        else
            printf "\r [✗]  Installing Factory AI CLI (Failed!)\n"
            cat /tmp/factory_install_$$.log
            rm -f /tmp/factory_install_$$.log
            return 1
        fi
        rm -f /tmp/factory_install_$$.log
    fi
